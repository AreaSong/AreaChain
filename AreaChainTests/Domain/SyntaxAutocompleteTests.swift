...
struct SyntaxAutocompleteTests {
    @Test func searchCandidatesNeverOfferCreationAndOnlyExposeSupportedAttributes() {
        ...
        #expect(SyntaxAutocompleteEngine.candidates(for: time, context: .search).allSatisfy { !$0.isCreation })
        #expect(SyntaxAutocompleteEngine.candidates(for: time, context: .tagSearch).isEmpty)
