...
struct InputSyntaxInteractionTests {
    @Test func mouseCompletionUpdatesTheMultilineEditorWithoutWriting() async throws {
        ...
        try await host.settle()
        let completion = try #require((editor.delegate as? DaybookTextEditor.Coordinator)?.parent.autocomplete)
        #expect(completion.candidates.first { $0.id == "tag_工作" }?.insertText == "#工作 ")
        try host.snapshot("diary-mouse-completion")
        try host.click(NativeSyntaxUI.center("syntax.candidate.tag_工作", in: host.window))
        try await host.settle()
        #expect(editor.string == "#工作 ",
                "binding=\((editor.delegate as? DaybookTextEditor.Coordinator)?.parent.text ?? "nil"), key=\(host.window.isKeyWindow)")
    ...
    @Test func subtaskInputCreatesItsOwnTagAndDisplaysIt() async throws {
        ...
        try await host.settle()
        #expect(editor.string == "#今日 子任务内容")
        #expect(field.stringValue == "#今日 子任务内容")
        #expect(!editor.hasMarkedText())
        #expect(host.window.firstResponder === editor)
        #expect((field.delegate as? DaybookTextField.Coordinator)?.parent.autocomplete?.isActive == false)
        #expect(host.tags.isEmpty)
        try host.pressReturn()
        try await host.settle()
